...
- **User management — PARTIAL DONE (2026-07-13).** `/settings/users` (admin-gated): list users, assign/remove roles, toggle is_premium / is_extreme pool flags (admin-guarded actions + audit user.role.assign/remove, user.premium/extreme.set). This unblocks the premium/extrême routing tiers (previously no UI flagged users into the pools). **Invite-by-email DONE too** (2026-07-13): `inviteUser` action uses service-role `auth.admin.generateLink({type:'invite'})` + Brevo (`buildInviteEmail`) to email an activation link; the auth trigger provisions the public.users row, then the admin assigns roles from the table. Untested live (needs service-role key + verified Brevo sender).
