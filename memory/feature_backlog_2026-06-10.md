...
- **Templates emails/SMS:** photo-request (mail + SMS) DONE in `lib/templates.ts`; **facture finale accompagnement** mail builder DONE (`buildFinalInvoiceEmail`) but not yet wired to a send button. Intervention-confirmation mail already DONE.
- **Commercial extrême = routing tier** (confirmed): `is_extreme` flag added to leads; routing rule wiring still TODO.
